  ...
  async function buscarUsuarioPorId() {
    try {
      ...
      if (error.toString().includes("401")) {
        ...
        ToastAlerta("Usuário não encontrado!" , 'error')
        retornar()
      }
    }
  }
    ...
    if (confirmarSenha === user.senha && user.senha.length >= 8) {
      try {
        ...
        if (error.toString().includes("401")) {
          ...
          ToastAlerta("Erro ao atualizar o usuário!", 'error')
          retornar()
        }
      }
    } else {
      ToastAlerta("Dados inconsistentes. Verifique as informações do usuário.", 'error')
      setUser({ ...user, senha: "" })
      setConfirmarSenha("")
    }
